...
echo Telepítem a requirements.txt csomagokat...
pip install -r requirements.txt
if errorlevel 1 (
    echo Hiba: A csomagok telepítése sikertelen volt!
    echo Kérlek próbáld meg újra futtatni a scriptet.
    pause
    exit /b 1
)
echo.

REM Ellenőrizzük a kritikus csomagok telepítését
echo Ellenőrzöm a kritikus csomagok telepítését...
python -c "import flask; import jwt; import humanize; import flask_sqlalchemy; import flask_login; import flask_wtf; import flask_limiter; import dotenv" > nul 2>&1
if errorlevel 1 (
    echo Hiba: Egy vagy több kritikus csomag nincs megfelelően telepítve!
    echo Kérlek próbáld meg újra futtatni a scriptet.
    pause
    exit /b 1
)
